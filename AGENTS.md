...
| **Siedler / sibling-service contract** (`/llm/kg`, `/llm/manifest.json`, `/llm.txt`, `/api/v1/prewarm`, `/api/v1/trees/bbox`, `/api/v1/query/buildings`, slim parcel fields, `kg_trees`, footprint match) | `docs/siedler-contract.md` | touching `llm_api.py`, `building_footprint_match.py`, the siedler AHEAD list, `kg_parcels.frav`, bbox parcel queries |
